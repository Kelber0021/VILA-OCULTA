...
export function Portrait({ id, name = "", className = "" }: { id: string; name?: string; className?: string }) {
  ...
  return <span className={`portrait-image ${className}`}><Image src={`/assets/characters/${validId}-desenho.png`} alt={name} fill sizes="(max-width: 640px) 33vw, 200px" /></span>;
}
export function PortraitPicker({ value, onChange, compact = false, disabled = false }: { value: string; onChange: (id: string) => void; compact?: boolean; disabled?: boolean }) {
  return <div className={`portrait-picker ${compact ? "compact" : ""}`} role="group" aria-label="Escolher retrato do personagem">
